...
export const ProfileModal = () => {
  ...
  const { profile } = data.member as { profile: Profile }
  const { currentMember, member } = data as { currentMember: Member; member: Member & { profile: Profile } }
  ...
  const initiateConversation = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/conversation', {
        ...
        body: JSON.stringify({
          conversationFrom: currentMember.profileId,
          conversationTo: member.profileId,
        }),
      })
